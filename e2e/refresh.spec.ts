...
async function mockAllRoutes(page: Page): Promise<void> {
  await page.route('**/regional/postcode/**', (r) =>
    r.fulfill(jsonResponse(CI_BODY)),
  );
  ...
  await page.route('**/standard-unit-rates/**', (r) =>
    r.fulfill(
      jsonResponse(JSON.stringify({ results: [{ value_inc_vat: 24.5 }] })),
    ),
  );
  await page.route('**/postcodes/**', (r) =>
    r.fulfill(
      jsonResponse(
        JSON.stringify({ result: { latitude: 52.628, longitude: 1.298 } }),
      ),
    ),
  );
...
test('refresh: clicking refresh refreshes UI', async ({ page }) => {
  await page.goto('/');
  await page.evaluate(() => {
  ...
  await expect(page.getByTestId('carbon-intensity-card')).toBeVisible({
    timeout: 10_000,
  });
