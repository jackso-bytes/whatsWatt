...
5. Run 'pnpm exec eslint .' and 'npx jest --config jest.config.cjs' and confirm BOTH pass with zero errors before continuing. Fix any failures before opening a PR — do not open a PR with failing lint or tests. NEVER add files to the eslint ignores list to silence errors — fix the root cause instead. NEVER leave camelCase stub files alongside kebab-case canonical files. \
