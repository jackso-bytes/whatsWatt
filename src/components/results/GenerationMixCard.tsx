...
function lcoeEntry(lcoe: Record<string, LcoeEntry>, fuel: string): LcoeEntry | undefined {
  return Object.entries(lcoe).find(([k]) => k === fuel)?.[1]
}
...
function buildDonutAriaLabel(sorted: FuelEntry[], lcoe: Record<string, LcoeEntry>): string {
  const parts = sorted.map(({ fuel, perc }) => `${fuelLabel(fuel, lcoeEntry(lcoe, fuel))} ${perc}%`)
...
function buildSegments(sorted: FuelEntry[], lcoe: Record<string, LcoeEntry>): Segment[] {
  ...
  return sorted.map(({ fuel, perc }) => {
    const colour = lcoeEntry(lcoe, fuel)?.colour ?? '#888'
...
const SPEC_FUELS = new Set(['wind', 'gas', 'nuclear', 'solar', 'biomass', 'imports', 'hydro'])
...
const FOOTER_STYLE: CSSProperties = { fontSize: 'var(--text-xs)', color: 'var(--color-text-disabled)', marginTop: 'var(--space-md)', borderTop: '1px solid var(--color-border)', paddingTop: 'var(--space-md)', lineHeight: '1.6' }
...
function DonutCentreLabel({ dominant, label }: Readonly<CentreProperties>) {
  return (
    <div aria-hidden="true" style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
      <span style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--color-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', lineHeight: '1.2' }}>{label}</span>
      <span style={{ fontFamily: 'var(--font-display)', fontSize: '1.4rem', fontWeight: 800, color: 'var(--color-text-primary)', fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.03em', lineHeight: '1.1' }}>{dominant?.perc}%</span>
    </div>
  )
}
...
function DonutChart({ sorted, lcoe }: DonutProperties) {
  ...
  const dominantLabel = dominant ? fuelLabel(dominant.fuel, lcoeEntry(lcoe, dominant.fuel)) : ''
  return (
...
function LegendItem({ fuel, perc, entry }: LegendItemProperties) {
  ...
  return (
    <li role="listitem" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}>
      ...
      <span className="legend-fuel" style={{ flex: 1, fontSize: 'var(--text-sm)', fontWeight: 500, color: 'var(--color-text-secondary)', minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</span>
...
export function GenerationMixCard({ mix, lcoe, error, onRetry }: Properties) {
  ...
  const sorted = mix.filter(({ fuel }) => SPEC_FUELS.has(fuel)).toSorted((a, b) => b.perc - a.perc)
  return (
    <article data-testid="generation-mix-card" style={CARD_STYLE}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-lg)' }}>
        ...
        <ul role="list" aria-label="Generation mix legend" style={LEGEND_STYLE}>
          {sorted.map(({ fuel, perc }) => <LegendItem key={fuel} fuel={fuel} perc={perc} entry={lcoeEntry(lcoe, fuel)} />)}
        </ul>
      </div>
