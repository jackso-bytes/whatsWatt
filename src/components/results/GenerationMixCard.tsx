...
const ALLOWED_FUELS = new Set(['wind', 'gas', 'nuclear', 'solar', 'biomass', 'imports', 'hydro']);
...
export function GenerationMixCard({ mix, lcoe, error, onRetry }: Properties) {
  ...
  const filtered = mix.filter(({ fuel }) => ALLOWED_FUELS.has(fuel));
  const sorted = filtered.toSorted((a, b) => b.perc - a.perc);
  return (
